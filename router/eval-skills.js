// skill 路由评估器：在 70 个个人 skill 上跑测试集，算 precision/recall + 阈值扫描。
// 与 eval.js 同结构，语料换成 skills-corpus.json（frontmatter descriptor）。
...
const OUT = path.join(__dirname, '..', 'results', 'skills-l0-baseline.json');
...
function macro(rows) {
  ...
  return {
    precision: sum.p / rows.length,
    recall: sum.r / rows.length,
    f1: sum.f / rows.length,
  };
}
...
function main() {
  ...
  const result = {
    ...
    router: 'L0 keyword, weighted-coverage + idf, CJK bigram, descriptor=description+triggers+name',
  ...
  console.log('=== skill 路由器评估 ===');
  console.log(`语料: ${result.corpus}`);
