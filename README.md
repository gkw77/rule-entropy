...
>
> 状态：MVP（L0 关键词 + L1 语义判定，rule & skill 双语料已验）。clone 即跑。L0 零依赖；L1 复用终端 LLM 环境变量。
...
rule 语料（13 文件，中英混合标题）L0 缺口主要在 precision。但真实规模语料--作者的 70 个个人 skill（1143 SKILL.md 的子集），description 多为英文--L0 撞上**语言墙**：中文 query 与英文 description 关键词零重叠，20 题里 10 题零匹配，L0 skill baseline 只有 P=0.304 / R=0.500 / F1=0.349。这是关键词层的天花板，正是 L1 跨语言该治的。
