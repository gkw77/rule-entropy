...
>
> 状态：MVP（L0 关键词 + L1 语义判定，rule & skill 双语料已验，含 70->325 规模效应 + recall悬崖/precision退化双修法 + skill 评分去重合并 receipt）。clone 即跑。L0 零依赖；L1 复用终端 LLM 环境变量。
...
路由是**结构性减熵**（不全载）；评分 + 去重是**赘余性减熵**（剪重复低质）。第二条做功。skill 每天更新增加（熵增），只增不减就堆积重复低质--给每个 skill 评分 + 找语义重复，才能识别剪枝候选。咬合项目立论的两条减熵做功。

**片 1：评分（零 LLM，便宜可观测白盒）** -- 325 skill 三维分，复合 = 0.45×完整性 + 0.40×独特性 + 0.15×新鲜度：
- **完整性**：有 description + 够长(>=30字) + 有 triggers
- **新鲜度**：mtime 距今天数（熵信号）
- **独特性**：描述 token 的 avg IDF（低 = 和别的 skill 重叠 = 赘余候选）

结果：
- **只 3/325 真·破损**（无描述：project-guidelines-example / browser-qa / math-olympiad）--语料整体质量不错，分布双峰：3 个 <0.2，其余 322 个在 0.6-1.0。
- **新鲜度不区分**（全 2026-04~06 安装，反映 install 时间非维护时间）--诚实：本快照 freshness 维度无效；它在长生命周期环境（skill 跨年增删）才有区分力。
- **source 对比**：marketplace 独特性最高（0.752，描述具体），cache 最低（0.568，泛泛）；marketplace/cache 完整率 0.00 是**格式约定**（它们不用 `triggers` frontmatter 字段）非质量差--评分有格式偏差，已标注。
- **IDF-独特性只抓到 3 个破损的，抓不到语义重复**（不同名但同功能）-> 需片 2。
- 顺手修了 manifest 解析器 bug：YAML 块标量 `description: |`（gstack/ctx-* 用）之前被当成描述字面值 `|`，现在正确解析多行。

**片 2：语义去重（LLM 批量分组）** -- 325 skill 喂 glm-5.2（关 thinking，去重只要 JSON 不用推理）一次性分组"功能重复"的：
- **18 重复组，32 冗余候选（9.8%）**。
- **真重复（高置信）**：版本重复（continuous-learning v1/v2）、格式变体（gstack-openclaw-* 4 个 = 个人 skill 的 openclaw 改名版，是 name 去重没塌缩掉的跨名副本）、约定拆分（everything-claude-code / -conventions）、MCP builder 系（mcp-builder / build-mcp-server / build-mcp-app / build-mcpb）、TDD 系、skill-creator 系。
...
## 第九个 receipt：同类项两两合并（无损减熵，替代剔除）

第七个 receipt 把 22 个冗余 skill 标为"剔除候选"。但**直接剔除会丢信息**--被删的那个可能有 canonical 没有的内容（不同触发面、不同措辞的描述）。用户修正：**不该剔，该合并**--同类项两两合并，取并集，count 减少但信息不丢。这把减熵立论说得更精确：**减熵不是删，是合并同类项降冗余（无损）**。

**做法**：对 dedup-verify 的 16 组确认重复，每组两两 LLM 合并描述（并集、去重重叠措辞、不丢任何一方独有信息）+ triggers 取并集，保留 canonical 名（最高分）。组 >2 时迭代 fold（canonical 为基，redundant 逐个并入）--"两两"。

**结果：38 skill -> 16 合并 skill（22 冗余无损并入 canonical，22 次合并调用）**。示例：
- `review` + `code-review-and-quality` -> "Pre-landing (pre-merge) PR review (gstack). Conducts multi-axis code review..."（两者功能并集）
- `skill-creator` + `write-a-skill` + `skill-development`（三路两两 fold）-> 合并三者"创建/写入/改进 skill"功能
- `continuous-learning-v2` + `continuous-learning`（两个日文描述）-> 跨语言合并也成立
- `mcp-builder` + `build-mcp-server` -> guide + "use when build MCP server" 触发面并集

**更精确的处置结论**（替代第七个 receipt 的"25 剪枝候选"）：
- **22 冗余 -> 合并入 16 canonical**（无损并集，信息不丢）--这是默认处置
- **3 真·破损**（无描述：project-guidelines-example / browser-qa / math-olympiad）-> 才该删（没内容可合并）

**边界**：只合并了 frontmatter（description + triggers），**body 合并未做**--SKILL.md 正文长且结构化，body 合并需人判或更复杂 LLM，留后续。部署（写合并后 SKILL.md 到 `~/.claude`、替换原 N 个）是破坏性，交人拍板，本 receipt 只产出合并描述。详见 `results/skills-merged.json`。
...
**自指示范**：作者审自己的 177 个规则块，89% behavior（N/A），剩 ~19 metric-adjacent **全部 secondhand/faith，0 真自测**。建仪器后破 2 个 P0（gzh 双关卡独立 rig、agent-chief 数字复现），路由器是**第 3 个 receipt**--且是唯一测"自己规则"而非"别人数字"的那个。路由器自身现已积累 14 个独立 rig receipt（L0 baseline/v2/v3证伪、L1 rule、L1 skill、L1 recall证伪、L0+L1 规模效应、覆盖度、规模悬崖修法、skill 评分、skill 语义去重、去重逐对确认、precision 规模退化修法、同类项两两合并），正负皆有。
...
router/skill-merge.js    同类项两两合并（无损减熵）：确认重复组两两 LLM 合并描述+triggers 并集，替代剔除
...
- [x] **同类项两两合并（无损减熵）**：22 冗余合并入 16 canonical（并集描述不丢信息），替代剔除；3 真破损才删。见上方第九个 receipt
